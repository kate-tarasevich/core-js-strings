...
function getStringLength(value) {
  let result;
  if (value !== null && value !== undefined) result = value.length;
  else result = 0;
  return result;
}
...
function isString(value) {
  let result;
  if (typeof value === 'string' || value instanceof String) result = 1;
  else result = 0;
...
function isPalindrome(str) {
  ...
  if ([...string].reverse().join('') === string) result = 1;
  else result = 0;
  return Boolean(result);
}
...
function reverseWords(str) {
  ...
  let result = '';
  for (let i = 0; i < words.length; i += 1) {
    const reversedWord = [...words[i]].reverse().join('');
...
function extractNameFromTemplate(value) {
  const firstPart = value.indexOf('Hello, ') + 'Hello, '.length;
  const lastPart = value.lastIndexOf('!');
  const result = value.slice(firstPart, lastPart);
  return result;
}
